...
public class AlunoService {
    ...
    public void matricularDisciplina(String nomeDisciplina, Long idAluno) {
        ...
        if (!podeMatricular) {
            throw new IllegalStateException(
                    "Aluno não pode se matricular na disciplina devido a limites de matrícula.");
        }
    ...
    public void consultarDisciplinasCursadas(Long idAluno) {
        ...
        if (aluno.getDisciplinas().isEmpty()) {
            ...
            StringBuilder disciplinasCursadas = new StringBuilder(
                    "Disciplinas cursadas por " + aluno.getNome() + ":\n");
    ...
    public void efetuarMatricula(Long idAluno) {
        Aluno aluno = consultarAluno(idAluno); // Certifique-se de que este método está carregando todas as disciplinas
                                               // corretamente

        if (aluno.getDisciplinas().isEmpty()) {
            System.err.println("Não há disciplinas disponíveis para matrícula.");
        } else {
            // Use uma cópia da lista de disciplinas para evitar problemas de
            // ConcurrentModificationException ao remover disciplinas da lista original
            ...
            for (Disciplina disciplina : disciplinasParaVerificar) {
                if (disciplinaService.verificaStatusDisciplina(disciplina.getNome())) {
                    System.out.println(
                            "O aluno " + aluno.getNome() + " foi matriculado na disciplina " + disciplina.getNome());
                } else {
                    cancelarMatriculaDisciplina(disciplina.getNome(), idAluno);
                    System.out.println("O aluno " + aluno.getNome() + " não pode se matricular na disciplina "
                            + disciplina.getNome()
                            + " pois ela não atingiu a quantidade de alunos necessários para ocorrer neste semestre.");
                }
            }
        }
    ...
    public double calcularMensalidade(Long alunoId) {
        Aluno aluno = consultarAluno(alunoId);
        double mensalidade = 0;
        
        for (int i = 0; i < aluno.getDisciplinas().size(); i++) {
            mensalidade += aluno.getDisciplinas().get(i).getPreco();
        }
